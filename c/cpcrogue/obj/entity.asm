...
;src/entity.c:78: e->hp -= dmg;
	...
	inc	hl
	ld	(hl), d
;src/entity.c:79: PrintAt (STATUS_X,STATUS_Y+1, e->name, PEN_BRIGHT);
	ld	hl, #0x0006
	add	hl, bc
	ld	a, #0x02
	push	af
	...
	pop	af
	inc	sp
;src/entity.c:80: PrintAt (STATUS_X+7,STATUS_Y+1, "takes     ", PEN_NORMAL);
	ld	a, #0x03
	push	af
	inc	sp
	ld	hl, #___str_0
	...
	call	_PrintAt
	pop	af
;src/entity.c:81: PrintU8 (dmg, STATUS_X+13,STATUS_Y+1, PEN_BRIGHT);
	inc	sp
	ld	hl,#0x0216
	ex	(sp),hl
	ld	a, #0x0f
	...
	call	_PrintU8
	pop	af
;src/entity.c:82: PrintAt (STATUS_X+16,STATUS_Y+1, "points of damage", PEN_NORMAL);
	ld	h,#0x03
	ex	(sp),hl
	inc	sp
	ld	hl, #___str_1
...
_EntityAttack::
	...
	ld	ix,#0
	add	ix,sp
;src/entity.c:92: PrintAt (STATUS_X,STATUS_Y, e->name, PEN_BRIGHT);
	ld	e,4 (ix)
	ld	d,5 (ix)
	ld	hl, #0x0006
	add	hl, de
	...
	inc	sp
	pop	de
;src/entity.c:94: PrintAt (STATUS_X+16,STATUS_Y, target->name, PEN_BRIGHT);
	ld	c,6 (ix)
	ld	b,7 (ix)
	ld	hl, #0x0006
	add	hl, bc
	...
	ld	d, l
	pop	bc
;src/entity.c:97: EntityTakeDamage (target, dmg);
	push	de
	inc	sp
	push	bc
...
___str_2:
	.ascii "attacks"
	.db 0x00
;src/entity.c:102: u8 GetBlockingEntity (TEntity *entities[], TEntity **out_e, u8 x, u8 y)
;	---------------------------------
...
_GetBlockingEntity::
	...
	add	ix,sp
	push	af
;src/entity.c:107: while ( (e=*entities++) != NULL) {
	ld	c,4 (ix)
	ld	b,5 (ix)
00105$:
	ld	l, c
	...
	or	a,e
	jr	Z,00107$
;src/entity.c:108: if (e->blocks && e->x == x && e->y == y) {
	pop	hl
	push	hl
	ld	de, #0x0015
	add	hl, de
...
;src/entity.c:109: *out_e = e;
	...
	ld	a, -1 (ix)
	ld	(bc), a
;src/entity.c:110: return TRUE;
	ld	l, #0x01
	jr	00108$
00107$:
;src/entity.c:113: return FALSE;
	ld	l, #0x00
00108$:
	ld	sp, ix
	pop	ix
	ret
;src/entity.c:118: void EntityDrawEntities (TEntity *entities[], u8 dirty[])
;	---------------------------------
...
_EntityDrawEntities::
	...
	add	ix,sp
	push	af
;src/entity.c:122: while ( (e=entities[i]) ) {
	ld	c, #0x00
00103$:
	ld	l, c
	ld	h, #0x00
...
;src/entity.c:123: if (dirty[i]) {
	...
	or	a, a
	jr	Z,00102$
;src/entity.c:124: EntityDraw (e, TRUE);
	push	hl
	push	bc
	ld	a, #0x01
	push	af
	...
	pop	bc
	pop	hl
;src/entity.c:125: dirty[i] = FALSE;
	ld	(hl), #0x00
00102$:
;src/entity.c:127: EntityDraw (e, FALSE);
	push	bc
	xor	a, a
	push	af
	inc	sp
